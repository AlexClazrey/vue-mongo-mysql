...
-- begin attached script 'sample_data'
call add_board("Board 1", @bid1);
call add_board("Board 2", @bid2);

call user_register("alice", "Alice", "ali", "ssssssss", "alice@sample.com", @uid1);
call user_register("bob", "Bob", "bob", "dddddddddd", "bob@sample.com", @uid2);
call user_register("catdog", "Cat Dog", "catdog", "ccccccc", "catdog@sample.com", @uid3);

set @normal_group = (select id from `group` where `name` = "Normal user");
call add_user_to_group(@uid1, @normal_group, null);
call add_user_to_group(@uid2, @normal_group, null);
call add_user_to_group(@uid3, @normal_group, null);

call save_draft(@uid1, null, "First Post By Alice", "I'm Alice, I'm so happy today. I want to tell some stories.", @pid1);
call save_draft(@uid1, null, "Second Post By Alice", "A girl like me wanna find something interesting to do.", @pid2);
call add_post_to_board(@pid1, @bid1);
call add_post_to_reply(@pid2, @pid1, @success);
call commit_post(@uid1, @pid1);
call commit_post(@uid1, @pid2);

call save_draft(@uid2, null, "First Post By Bob", "Oh I can save post as a draft.", @pid3);
call save_draft(@uid2, null, "Second Post By Bob", "Try again.", @pid4);
call add_post_to_board(@pid3, @bid2);
call add_post_to_reply(@pid4, @pid1, @success);




-- end attached script 'sample_data'
